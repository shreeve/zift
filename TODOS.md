...
- [ ] **Per-handle access mode is not tracked.** `Handle` (`session.zig:542-549`) has no `can_read`/`can_write`. `handleRead` (`session.zig:717`) and `handleWrite` (`session.zig:735`) only check `kind == .file`. A client can `OPEN` write-only and then `READ`, or `OPEN` read-only and then `WRITE`. PLAN §6.3 names `read` and `write` as the access controls for `SSH_FXP_READ`/`SSH_FXP_WRITE`. Fix: derive `{can_read, can_write}` from the parsed open flags at OPEN time, store on the handle, enforce at every `handleRead`/`handleWrite`/`handleFstat`. Add tests.
- [ ] **`SSH_FXP_OPEN` write path is symlink-escape-prone before `verifyFile`.** `handleOpen` (`session.zig:681`) calls `std.Io.Dir.cwd().createFile(self.io, real, …)` on a path-string built from `resolveExisting`/`joinRoot`. If a symlink inside the jail points at `/etc/passwd`, `createFile` follows the symlink, creates/truncates the target, *then* `verifyFile` rejects the FD — too late. PLAN §8.3 guarantees symlink escapes do not modify state outside the jail. Fix: rewrite all OPEN paths to use `openVerifiedParent`-style FD-relative open with `O_NOFOLLOW` on the final component, then `verifyFile` before any truncation/write occurs. (`vfs.zig` already has the building blocks.)
- [ ] **`SSH_FXP_OPEN` does not honor `SSH_FXF_*` flags correctly.** `session.zig:681` always calls `createFile` with `.read = true, .truncate = (flags & TRUNC) != 0`, regardless of whether `CREAT` was requested, whether `EXCL` was requested, or whether `APPEND` was requested. PLAN §7.6 commits to ordinary SFTP v3 OPEN semantics. Fix: parse the SFTP open-flag bitmask (`READ`/`WRITE`/`APPEND`/`CREAT`/`TRUNC`/`EXCL`), translate to `O_*` flags, and use `openat`-style with explicit mode `0644` (PLAN §7.6).
- [ ] **`max-connections` doesn't bound pre-auth.** `session.zig:122-148` does `ssh_bind_accept` first, *then* checks the counter and disconnects if over. Until that disconnect lands, the slot has burned an `ssh_session` allocation, key-exchange potential, and one of the host's open-fd budget. PLAN §6.2 implies the limit bounds the slot count, not "post-handshake disconnects." Fix: gate `accept()` on counter, or perform the `ssh_session` allocation only after slot reservation, or accept-and-immediately-close at the raw-fd layer before any libssh state. Audit line stays the same.
- [ ] **No idle timeout pre-auth.** `c.ssh_handle_key_exchange` (`session.zig:357`), libssh password/pubkey message loop (`session.zig:413`), and `acceptSftpSubsystem` (`session.zig:474`) all block indefinitely. A client that connects and then sits there pins a worker thread and a `max-connections` slot forever. PLAN §6.2 says idle clients are disconnected. Fix: wrap the libssh handshake/auth/subsystem phases in a deadline-driven loop using libssh's nonblocking mode + `poll(ssh_get_fd(session))`, or set a libssh-level read timeout. Same `idle-timeout` value applies.
- [ ] **`zift validate` is a syntax-only check.** `main.zig:68-107` calls `config.parse` and prints "ok". PLAN §4.8 and §6.2 say validate must reject missing roots, unreadable host key, out-of-policy Argon2id parameters, **and overlapping roots**. Fix: introduce `config.validate(cfg)` that runs all semantic checks. Wire it into `validate` and into `serve` startup before listening. Same routine should run on reload.
- [ ] **Roots are not canonicalized or required to exist at config load.** `config.zig:236-238` accepts `root /tmp/whatever` even if the directory doesn't exist; failure surfaces only when a user logs in and `Vfs.init` realpaths the root in the worker. PLAN §6.2 says each root is canonicalized at config load. Fix: in the new `config.validate`, `realpath` every `root` and stat it as a directory. Store the canonical form alongside the original. Reload uses the same path.
- [ ] **Overlapping roots are not rejected.** PLAN §6.2 explicitly: "Two users with overlapping `root` paths is rejected." `config.zig` does no overlap check. Fix: after canonicalization, compare every pair of roots; reject if any is equal to or a path-component prefix of another. Cite the offending users + line numbers in the error.
