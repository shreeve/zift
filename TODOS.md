...
- [ ] **Audit lines emit before the SFTP reply, not after** (PLAN §8.5: "Every operation handler emits its audit line after replying to the client"). The current SftpState handlers call `auditDenied`/`auditOk`/`auditFailed` immediately before `replyStatus`, so a slow audit destination (file on a hung NFS mount) blocks the client's reply. Fix: flip every handler to reply first, audit second; `defer self.auditXxx(...)` is a clean one-line per call site. Sabotage test: configure `log /tmp/audit.fifo`, attach no reader, observe SFTP requests stall before the fix and complete after.
